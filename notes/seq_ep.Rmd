...
\begin{align*}
  p(x) &= N(x; 0, \Sigma) 1 \{l \le Cx \le u \}
  \\
  &= N(x; 0, \Sigma) \prod_{i=1}^m t_i(x) 
\end{align*}
...
Let $A_{\cdot, k}$ be the $k$th column of $A$. Let $\epsilon_i \sim_{iid} N(0, 1)$ for $i \in \{1, \ldots, n\}$. We construct a sequence of random
...
where
\begin{align*}
  p(x_i | x_{i-1}) &= N(x_i; x_{i-1}, A_{\cdot, i}A^T_{\cdot, i}) & i \in \{2, \ldots, n\}
  \\
  p(x_1) &= N(x_1; 0, A_{\cdot, 1}A^T_{\cdot, 1})
\end{align*}
The covariance $\Xi_i = A_{\cdot, i}A_{\cdot, i}^T$ is not invertible in general,
it is rank 1.
...
Only the bottom right $n \times n$ corner of $K^{-1}$, $\Xi_n^+$, will be involved in the 
EP updates. In general for a $n^2 \times n^2$ matrix $B$, denote that sub matrix as $B_{-n}$. 

We do not have easy access to $K$, which would require inverting the singular and $d^2 \times d^2$ dimensional precision $K^{-1}$. In EP updates we would like to work only with $K^{-1}$. 
