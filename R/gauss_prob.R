#' @export
pmvn <- function(mu, Sigma, lb, ub, A = diag(length(mu)), data_samples = NULL,
                 n_noise) {
...
#' @export
pmvn2 <- function(n, mu, Sigma, A, b, data, noise) {
...
# n <- 1000
# 
# samples <- TruncatedNormal::rtmvnorm(n, mu, Sigma, lb, ub)
# 
# ep_moments <- epmgp::moments(lb, ub, mu, Sigma)
# noise <- mvtnorm::rmvnorm(n, ep_moments$mu, ep_moments$Sigma)
# 
# A <- rbind(diag(d), -diag(d))
# b <- rbind(lb, -ub)
# 
# epmgp::pmvn2(mu, Sigma, lb, ub, diag(d))
